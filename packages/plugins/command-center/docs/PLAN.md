...
- **V2.1 — self-healing rework loop (BUILT):** a QA FAIL no longer stops at "needs rework" — the reducer (`src/brain/run.ts`) re-dispatches the task to its role with the QA findings baked into the brief, re-verifies, and repeats up to `maxRework` (default 2) before settling as failed. Bounded so it converges; the deterministic verdict stays authoritative. `src/qa/gate-scaffold.ts` emits the real Playwright + `@clerk/testing` + Midscene harness (config, Clerk setup, gate spec, advisory spec, CI) as a path→contents map so the gate is generatable, not just documented. A built-in `designer` role rounds out the roster.
